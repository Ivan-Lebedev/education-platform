...
class SqlAlchemyUnitOfWork(UnitOfWork):
    """
    SQLAlchemy-реализация контракта объекта 'UnitOfWork' для управления транзакциями БД.

    Поддерживает 2 режима создания:
    - с фабрикой сессий (session_factory): UoW сам открывает сессию при входе
      и закрывает её при выходе;
    - с внешней сессией (session): UoW работает с переданной сессией и не закрывает её.
      Жизненным циклом владеет передающая сторона (например, тестовая фикстура).
    """
